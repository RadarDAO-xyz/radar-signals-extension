...
function App() {
    ...
    const prevStage = () => {
        setStage(
            stages[stages.indexOf(stage) - 1 >= 0 ? stages.indexOf(stage) - 1 : stages.length - 1]
        );
    };
    ...
    useEffect(() => {
        if (token === '') {
            setLoading(true);
            chrome.runtime.sendMessage({ message: 'AUTHENTICATE', token }, function (response) {
                setLoading(false);
    ...
    return (
        <StyledApp>
            <div
                className="wrapper"
                style={{
                    backgroundImage: stage === 'success' ? `url(${bgSuccess})` : `url(${bg2})`
                }}
            >
